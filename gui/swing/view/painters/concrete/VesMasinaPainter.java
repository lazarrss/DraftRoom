...
public class VesMasinaPainter extends RoomElementPainter {
    public VesMasinaPainter(RoomElement roomElement) {
        super(roomElement);
...
//        }
        ...
        path.moveTo(x, y);
        path.append(new Rectangle(x, y, width, height), true);

        double precnikHeight = (double) (4 * height) / 6;
        double precnikWidth = (double) (4 * width) / 6;
        path.moveTo(x + precnikWidth / 6, y + precnikHeight / 6);
        Ellipse2D ellipse2D = new Ellipse2D.Double(path.getCurrentPoint().getX(), path.getCurrentPoint().getY(), precnikWidth * 7/6, precnikHeight * 7/6);
        path.append(ellipse2D, false);
        path.closePath();
        setShape(path);
        g.draw(getShape());
