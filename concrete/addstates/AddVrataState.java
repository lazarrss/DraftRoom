...
import raf.draft.dsw.model.structures.roomelements.concrete.Vrata;
...
public class AddVrataState implements State {
    @Override
    public void misKliknut(MyTabPanel roomView, Point point1) throws NoninvertibleTransformException {
        ...
        Vrata vrata = new Vrata("", roomView.getRoom(), point, width, height, 0);
        vrata.addSubscriber(roomView);
        MainFrame.getInstance().getTabbedPane().setRoomElement(vrata);

        VrataPainter vp = new VrataPainter(vrata);

        AbstractCommand abstractCommand = new AddCommand(roomView, vp);
