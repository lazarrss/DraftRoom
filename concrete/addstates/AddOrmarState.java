...
public class AddOrmarState implements State {
    ...
    public void misKliknut(MyTabPanel roomView, Point point1) throws NoninvertibleTransformException {
        ...
        Ormar ormar = new Ormar("", roomView.getRoom(), point, width, height, 0);
        ormar.addSubscriber(roomView);
        MainFrame.getInstance().getTabbedPane().setRoomElement(ormar);

        OrmarPainter op = new OrmarPainter(ormar);

        AbstractCommand abstractCommand = new AddCommand(roomView, op);
