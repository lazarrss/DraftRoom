...
import raf.draft.dsw.gui.swing.view.painters.concrete.StoPainter;
...
import raf.draft.dsw.model.structures.roomelements.concrete.Sto;
...
public class AddStoState implements State {
    @Override
    public void misKliknut(MyTabPanel roomView, Point point1) throws NoninvertibleTransformException {
        ...
        Sto sto = new Sto("", roomView.getRoom(), point, width, height, 0);
        sto.addSubscriber(roomView);
        MainFrame.getInstance().getTabbedPane().setRoomElement(sto);

        StoPainter sp = new StoPainter(sto);

        AbstractCommand abstractCommand = new AddCommand(roomView, sp);
