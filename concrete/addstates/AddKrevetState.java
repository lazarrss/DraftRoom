...
import raf.draft.dsw.gui.swing.view.painters.concrete.KrevetPainter;
...
public class AddKrevetState implements State {
    @Override
    public void misKliknut(MyTabPanel roomView, Point point1) throws NoninvertibleTransformException {
        ...
        Krevet krevet = new Krevet("", roomView.getRoom(), point, width, height, 0);
        krevet.addSubscriber(roomView);
        MainFrame.getInstance().getTabbedPane().setRoomElement(krevet);

        KrevetPainter kp = new KrevetPainter(krevet);
