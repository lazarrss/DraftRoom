...
import raf.draft.dsw.model.structures.roomelements.concrete.WCSolja;
...
public class AddWCSoljaState implements State {
    @Override
    public void misKliknut(MyTabPanel roomView, Point point1) throws NoninvertibleTransformException {
        ...
        WCSolja wcSolja = new WCSolja("", roomView.getRoom(), point, width, height, 0);
        wcSolja.addSubscriber(roomView);
        MainFrame.getInstance().getTabbedPane().setRoomElement(wcSolja);

        WCSoljaPainter wc = new WCSoljaPainter(wcSolja);

        AbstractCommand abstractCommand = new AddCommand(roomView, wc);
